...
export interface UseAutoAdvanceOptions {
  ...
  /**
   * Whether the carousel is visible (in viewport and tab is active)
   */
  isVisible: boolean;
...
/**
 * Custom hook that manages automatic image advancement for the carousel.
 * Handles timer management, pause conditions, and cleanup.
 *
 * Auto-advance will be paused when:
 * - User is interacting with the carousel (isPaused)
 * - Carousel is not visible in viewport or tab is not active (isVisible)
...
export function useAutoAdvance({
  ...
  }, [enabled, imagesCount, isPaused, isAnimating, isVisible]);
