...
export default function OrderDetailView({
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      ...
      try {
        ...
        setOrderError(null);
        console.log('[OrderDetailView] Fetching order:', orderId);
        const orderData = await orderApi.getOrder(orderId);
        console.log('[OrderDetailView] Order fetched successfully:', orderData.orderNumber);
        setOrder(orderData);
      } catch (error: any) {
        console.error('[OrderDetailView] Error fetching order:', {
          orderId,
  ...
  useEffect(() => {
    const fetchBankSettings = async () => {
      ...
      if (order && !showBankTransferForPaidOrders) {
      ...
      try {
        ...
        console.log('[OrderDetailView] Fetching bank transfer settings');
        const settings = await paymentSettingsApi.getBankTransferSettings();
        console.log('[OrderDetailView] Bank transfer settings fetched:', {
        ...
        console.error('[OrderDetailView] Error fetching bank transfer settings:', {
          status: error.response?.status,
          message: error.message,
          error: error.response?.data || error
        });
        ...
        if (error.response?.status === 404) {
          console.warn('[OrderDetailView] Bank transfer settings not configured');
          setBankSettings({
            accountName: '',
            accountNumber: '',
            bankName: '',
  ...
  }, [order, showBankTransferForPaidOrders, t]);
  ...
  const handleRetryOrder = () => {
    if (!orderId) {
      console.error('[OrderDetailView] Cannot retry: No order ID');
      return;
    }

    console.log('[OrderDetailView] Retrying order fetch:', orderId);
    setIsLoadingOrder(true);
    setOrderError(null);

    orderApi.getOrder(orderId)
      .then((orderData) => {
        console.log('[OrderDetailView] Order retry successful:', orderData.orderNumber);
        setOrder(orderData);
      })
      .catch((error) => {
        console.error('[OrderDetailView] Order retry failed:', {
          orderId,
          status: error.response?.status,
          message: error.message,
          error: error.response?.data || error
  ...
  const handleRetrySettings = () => {
    console.log('[OrderDetailView] Retrying bank transfer settings fetch');
    setIsLoadingSettings(true);
    setSettingsError(null);

    paymentSettingsApi.getBankTransferSettings()
      .then((settings) => {
        console.log('[OrderDetailView] Settings retry successful:', {
          hasAccountName: !!settings.accountName,
          hasAccountNumber: !!settings.accountNumber,
          hasBankName: !!settings.bankName,
          hasQrCode: !!settings.qrCodeUrl
      ...
      .catch((error) => {
        console.error('[OrderDetailView] Settings retry failed:', {
          status: error.response?.status,
          message: error.message,
          error: error.response?.data || error
        });
