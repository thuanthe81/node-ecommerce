...
/**
 * Shipping Section Output Test Script
 *
 * This script tests the shipping section HTML generation directly
...
async function testShippingSectionOutput() {
  console.log('🚀 Starting Shipping Section Output Test');
  ...
  const results: Array<{
    ...
    locale: 'en' | 'vi';
    success: boolean;
    documentStructureHTML?: string;
    templateEngineHTML?: string;
    error?: string;
  }> = [];
  ...
  for (const config of testShippingConfigurations) {
    ...
    for (const locale of ['en', 'vi'] as const) {
      ...
      try {
        ...
        // Test PDF Document Structure Service shipping section
        const documentStructureHTML = (documentStructureService as any).generateShippingSection(orderData, locale);

        // Test PDF Template Engine shipping section
        const templateEngineHTML = (templateEngine as any).generateShippingInfoHTML(orderData, locale);

        // Save the shipping section HTML for analysis
        const docStructureFileName = `${config.name}-${locale}-document-structure.html`;
        const templateEngineFileName = `${config.name}-${locale}-template-engine.html`;

        const docStructurePath = path.join(outputDir, docStructureFileName);
        const templateEnginePath = path.join(outputDir, templateEngineFileName);

        fs.writeFileSync(docStructurePath, documentStructureHTML);
        fs.writeFileSync(templateEnginePath, templateEngineHTML);

        console.log(`   ✅ Generated: ${docStructureFileName} & ${templateEngineFileName}`);
  ...
  const reportPath = path.join(outputDir, 'shipping-section-test-report.json');
  const report = {
...
async function analyzeTranslationConsistency() {
  ...
  for (const locale of ['en', 'vi'] as const) {
    for (const service of ['document-structure', 'template-engine'] as const) {
      console.log(`\n🌐 Analyzing ${locale} translations for ${service}...`);
      ...
      // Read all shipping HTML files for this locale and service
      const htmlFiles = fs.readdirSync(outputDir)
        .filter(file => file.endsWith(`-${locale}-${service}.html`));
      ...
      for (const file of htmlFiles) {
        ...
        const patterns = {
          ...
          description: content.match(/class="shipping-label"[^>]*>([^<]*Description[^<]*)/i),
          estimatedDelivery: content.match(/class="shipping-label"[^>]*>([^<]*Delivery[^<]*)/i),
          trackingNumber: content.match(/class="shipping-label"[^>]*>([^<]*Tracking[^<]*)/i),
          carrier: content.match(/class="shipping-label"[^>]*>([^<]*Carrier[^<]*)/i)
        };
  ...
  for (const locale of ['en', 'vi'] as const) {
    const docStructure = analysisResults.find(r => r.locale === locale && r.service === 'document-structure');
    const templateEngine = analysisResults.find(r => r.locale === locale && r.service === 'template-engine');

    if (docStructure && templateEngine) {
      const crossServiceInconsistencies: string[] = [];

      Object.keys(docStructure.translationKeys).forEach(key => {
        const docValues = [...new Set(docStructure.translationKeys[key])];
        const templateValues = [...new Set(templateEngine.translationKeys[key])];

        if (docValues.length > 0 && templateValues.length > 0) {
          const allValues = [...new Set([...docValues, ...templateValues])];
          if (allValues.length > 1) {
            crossServiceInconsistencies.push(`${key}: Document Structure uses "${docValues.join(', ')}" while Template Engine uses "${templateValues.join(', ')}"`);
          }
        }
      });

      console.log(`   ${locale.toUpperCase()}: ${crossServiceInconsistencies.length} cross-service inconsistencies`);
      if (crossServiceInconsistencies.length > 0) {
        crossServiceInconsistencies.forEach(inc => console.log(`   ⚠️  ${inc}`));
      } else {
        console.log(`   ✅ Services are consistent`);
      }
    }
  }

  // Save analysis report
  const analysisPath = path.join(outputDir, 'shipping-translation-analysis.json');
...
if (require.main === module) {
  testShippingSectionOutput()
    .then(analyzeTranslationConsistency)
    .then(() => {
      console.log('\n🎉 Shipping Section Output Test completed!');
      process.exit(0);
    })
    .catch(error => {
      console.error('\n💥 Test failed:', error);
...
export { testShippingSectionOutput, analyzeTranslationConsistency };