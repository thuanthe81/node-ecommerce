...
describe('TranslationService', () => {
  ...
  describe('translateOrderStatus', () => {
    it('should translate order statuses to English', () => {
      expect(service.translateOrderStatus(STATUS.ORDER_STATUS.PENDING, 'en')).toBe('Pending');
      expect(service.translateOrderStatus(STATUS.ORDER_STATUS.PENDING_QUOTE, 'en')).toBe('Pending Quote');
      expect(service.translateOrderStatus(STATUS.ORDER_STATUS.PROCESSING, 'en')).toBe('Processing');
      expect(service.translateOrderStatus(STATUS.ORDER_STATUS.SHIPPED, 'en')).toBe('Shipped');
      expect(service.translateOrderStatus(STATUS.ORDER_STATUS.DELIVERED, 'en')).toBe('Delivered');
      expect(service.translateOrderStatus(STATUS.ORDER_STATUS.CANCELLED, 'en')).toBe('Cancelled');
      expect(service.translateOrderStatus(STATUS.ORDER_STATUS.REFUNDED, 'en')).toBe('Refunded');
    });

    it('should translate order statuses to Vietnamese', () => {
      expect(service.translateOrderStatus(STATUS.ORDER_STATUS.PENDING, 'vi')).toBe('Chờ xử lý');
      expect(service.translateOrderStatus(STATUS.ORDER_STATUS.PENDING_QUOTE, 'vi')).toBe('Chờ báo giá');
      expect(service.translateOrderStatus(STATUS.ORDER_STATUS.PROCESSING, 'vi')).toBe('Đang xử lý');
      expect(service.translateOrderStatus(STATUS.ORDER_STATUS.SHIPPED, 'vi')).toBe('Đã giao vận');
      expect(service.translateOrderStatus(STATUS.ORDER_STATUS.DELIVERED, 'vi')).toBe('Đã giao hàng');
      expect(service.translateOrderStatus(STATUS.ORDER_STATUS.CANCELLED, 'vi')).toBe('Đã hủy');
      expect(service.translateOrderStatus(STATUS.ORDER_STATUS.REFUNDED, 'vi')).toBe('Đã hoàn tiền');
    });
  ...
  describe('translatePaymentStatus', () => {
    it('should translate payment statuses to English', () => {
      expect(service.translatePaymentStatus(STATUS.PAYMENT_STATUS.PENDING, 'en')).toBe('Pending');
      expect(service.translatePaymentStatus(STATUS.PAYMENT_STATUS.PAID, 'en')).toBe('Paid');
      expect(service.translatePaymentStatus(STATUS.PAYMENT_STATUS.FAILED, 'en')).toBe('Failed');
      expect(service.translatePaymentStatus(STATUS.PAYMENT_STATUS.REFUNDED, 'en')).toBe('Refunded');
    });

    it('should translate payment statuses to Vietnamese', () => {
      expect(service.translatePaymentStatus(STATUS.PAYMENT_STATUS.PENDING, 'vi')).toBe('Chờ thanh toán');
      expect(service.translatePaymentStatus(STATUS.PAYMENT_STATUS.PAID, 'vi')).toBe('Đã thanh toán');
      expect(service.translatePaymentStatus(STATUS.PAYMENT_STATUS.FAILED, 'vi')).toBe('Thất bại');
      expect(service.translatePaymentStatus(STATUS.PAYMENT_STATUS.REFUNDED, 'vi')).toBe('Đã hoàn tiền');
    });
  });
  ...
  describe('translateShippingMethod', () => {
    it('should translate shipping methods to English', () => {
      expect(service.translateShippingMethod('standard', 'en')).toBe('Standard Shipping');
      expect(service.translateShippingMethod('express', 'en')).toBe('Express Shipping');
      expect(service.translateShippingMethod('overnight', 'en')).toBe('Overnight Shipping');
      expect(service.translateShippingMethod('free', 'en')).toBe('Free Shipping');
    });

    it('should translate shipping methods to Vietnamese', () => {
      expect(service.translateShippingMethod('standard', 'vi')).toBe('Vận chuyển tiêu chuẩn');
      expect(service.translateShippingMethod('express', 'vi')).toBe('Vận chuyển nhanh');
      expect(service.translateShippingMethod('overnight', 'vi')).toBe('Vận chuyển qua đêm');
      expect(service.translateShippingMethod('free', 'vi')).toBe('Miễn phí vận chuyển');
    });

    it('should handle different formats', () => {
      expect(service.translateShippingMethod('Standard Shipping', 'en')).toBe('Standard Shipping');
      expect(service.translateShippingMethod('standard-shipping', 'en')).toBe('Standard Shipping');
      expect(service.translateShippingMethod('standardshipping', 'en')).toBe('Standard Shipping');
    });
  ...
  describe('getShippingMethodDescription', () => {
    it('should return descriptions with delivery estimates', () => {
      expect(service.getShippingMethodDescription('standard', 'en')).toContain('3-5 business days');
      expect(service.getShippingMethodDescription('express', 'en')).toContain('1-2 business days');
      expect(service.getShippingMethodDescription('overnight', 'en')).toContain('same day');

      expect(service.getShippingMethodDescription('standard', 'vi')).toContain('3-5 ngày làm việc');
      expect(service.getShippingMethodDescription('express', 'vi')).toContain('1-2 ngày làm việc');
    });
  });
