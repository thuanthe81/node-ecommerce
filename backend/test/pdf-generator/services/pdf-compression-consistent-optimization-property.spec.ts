    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (params) => {
          ...
          // Mock the optimizeImageForPDF method to simulate consistent processing
          const optimizeImageSpy = jest.spyOn(service as any, 'optimizeImageForPDF');
          optimizeImageSpy.mockImplementation(async (imageUrl: string, contentType: string) => {
            const buffer = imageBuffers.get(imageUrl) || imageBuffers.values().next().value;
            ...
            return {
              ...
              metadata: {
                ...
                qualityUsed: qualityMin + (qualityMax - qualityMin) * 0.5,
